...
// Game seed data - single source of truth
const GAMES_SEED = [
  ...
  { name: 'Nmines', slug: 'nmines', image: '/games/nmines.avif', description: 'Detecteur de mines avance Nmines', color: 'bg-yellow-600', icon: 'Bomb', unlockType: 'free', unlockValue: 0, tier: 1, sortOrder: 19, showOnLanding: false },
];

// Track sync state to avoid running on every request
let gamesSynced = false;
let gamesSyncPromise: Promise<void> | null = null;

// Auto-seed games (syncs DB with code, runs once per server instance)
export async function ensureGamesSeeded(): Promise<void> {
  if (gamesSynced) return;
  if (gamesSyncPromise) { await gamesSyncPromise; return; }
  ...
  gamesSyncPromise = (async () => {
    try {
      ...
      gamesSyncPromise = null; // Allow retry on failure
      throw e;
    }
  })();

  await gamesSyncPromise;
}
